...
class MyPAData(BaseData):
    """
    仅用 load_all / load_single 的最小数据类。
    每个 CSV = 1 个样本；样本 ID = 文件名去扩展名；label 来自 individual_attributes.csv 的 BMI 列。
    必备成员：all_df, feature_df, feature_names, all_IDs, max_seq_len, labels_df
    """
    def __init__(self, root_dir, file_list=None, pattern=None, n_proc=1, limit_size=None, config=None):
    ...
    def load_single(self, filepath):
        ...
        df.insert(0, "ID", sid)  # 先加 ID 列，__init__ 里再 set_index
        # 简单插值/填补
